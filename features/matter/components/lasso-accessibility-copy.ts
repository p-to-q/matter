...
const COPY: Readonly<Record<CanvasLanguage, LassoAccessibilityCopy>> = Object.freeze({
  ...
  "zh-CN": Object.freeze({
    ...
    groupInstructions: "两个握点控制同一个展开程度。上握点向上拉，保持上边界并向下打开所选文字及其后的内容；下握点向下拉，只移动所选文字后的内容。松手后可轻点选中框内确认。上箭头或右箭头增加，下箭头或左箭头减少；Page Up 和 Page Down 大步调整；Home 归零，End 调至最大，回车或空格应用，Escape 重置。",
  ...
  "zh-TW": Object.freeze({
    ...
    groupInstructions: "兩個握點控制同一個展開程度。上握點向上拉，保持上邊界並向下打開所選文字及其後的內容；下握點向下拉，只移動所選文字後的內容。放開後可輕點選取框內確認。上方向鍵或右方向鍵增加，下方向鍵或左方向鍵減少；Page Up 和 Page Down 大幅調整；Home 歸零，End 調至最大，Enter 或空白鍵套用，Escape 重設。",
  ...
  "ja-JP": Object.freeze({
    ...
    groupInstructions: "2つのグリップは同じ展開量を操作します。上のグリップは上へ引き、上の境界を固定したまま選択した言葉と後続の素材を下へ開きます。下のグリップは下へ引き、後続の素材だけを動かします。放した後は選択枠内をタップして確定できます。上または右矢印で増加、下または左矢印で減少します。Page Up と Page Down は大きく調整し、Home はゼロ、End は最大、Enter またはスペースで適用、Escape でリセットします。",
  ...
  "de-DE": Object.freeze({
    ...
    groupInstructions: "Beide Griffe steuern denselben Erweiterungsgrad. Der obere Griff wird nach oben gezogen, hält die obere Grenze fest und öffnet den ausgewählten Text mit allem Folgenden nach unten; der untere Griff wird nach unten gezogen und bewegt nur das Folgende. Nach dem Loslassen bestätigt ein Zeigerklick in die Auswahl. Pfeil nach oben oder rechts erhöht, nach unten oder links verringert. Bild auf und Bild ab ändern stärker. Pos1 setzt auf null, Ende auf das Maximum, Eingabe oder Leertaste wendet an und Escape setzt zurück.",
