...
describe("inquiry provider", () => {
  ...
  it("trims a complete provider answer without manufacturing a cut-off version", () => {
    const verdict = INQUIRY_SCENARIO.adjudicate(`  ${"答".repeat(1_300)}  `, request("材料"));
    expect(verdict.ok).toBe(true);
    if (!verdict.ok) return;
    expect(Array.from(verdict.value)).toHaveLength(1_300);
    expect(verdict.value.endsWith("…")).toBe(false);
    expect(INQUIRY_SCENARIO.adjudicate("答".repeat(3_201), request("材料")))
  ...
  ])("keeps a complete answer when only its presentation differs: %s", (answer) => {
    expect(INQUIRY_SCENARIO.adjudicate(answer, request("材料")))
      .toEqual({ ok: true, value: answer });
  });

  it("allows advice only when the person's question asks for it", () => {
    const prompt = compileInquiryPrompt({ ...request("材料"), question: "这段可以怎么改？" });
    expect(prompt).toContain("unless they asked for that");
  });
