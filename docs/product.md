...
direction and bounds. Its public deployment gate remains closed in Preview.40,
...
suffix and every material row that begins below it move down. The upper grip is
...
lineage     why it grew here      authored root → selected node
                         ↓
...
creates no transform request, and never enters tree, history, persistence, or
...
keep growing and eventually share.

```text
AI gives language intelligence.
Matter gives thought a body.
```

Everything exists so that a thought which has not finished forming can keep
growing.
